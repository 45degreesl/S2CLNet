...
from lib.contrastive import ContrastiveLoss
...
def get_dataset(image_set, transform, args):
    if args.dataset == "rrsisd":
        from data.rrsisd_refer_clip import ReferDataset
    else:
        from data.refsegrs_refer_clip import ReferDataset
...
def evaluate(model, data_loader, epoch):
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            output, x_c4, text_embeds = model(image, sentences, l_mask=attentions)
...
def train_one_epoch(model, contrastive_loss, criterion, optimizer, data_loader,
                    lr_scheduler, epoch, print_freq, iterations):
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    for i, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        total_its += 1
        image, target, sentences, attentions, save_prefix = data
        image, target, sentences, attentions = image.cuda(non_blocking=True),\
                                               target.cuda(non_blocking=True),\
                                               sentences.cuda(non_blocking=True),\
                                               attentions.cuda(non_blocking=True)
        ...
        optimizer.zero_grad()
        loss = criterion(output, target) + 0.35 * loss_contrast
...
def main(args):
    ...
    print("\n[***] Build Model")
    # print(args.model)
    model = segmentation.__dict__[args.model](pretrained=args.pretrained_clip_weights,
                                              args=args)
    model.cuda()
    ...
    params_to_optimize = [
        ...
        {"params": reduce(operator.concat,
                          [[p for p in model.text_encoder.transformer.resblocks[i].parameters()
                            if p.requires_grad] for i in range(12)])},
        {"params": [p for p in contrastive_loss.parameters() if p.requires_grad]},
    ]
    ...
    if args.resume:
        ...
        if 'contrastive_loss' in checkpoint:
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        train_one_epoch(model, contrastive_loss, criterion, optimizer, data_loader, lr_scheduler, epoch, args.print_freq,
                        iterations)
        iou, overallIoU = evaluate(model, data_loader_test, epoch)
