...
def get_dataset(image_set, transform, args):
    if args.dataset == "rrsisd":
        from data.rrsisd_refer import ReferDataset
    else:
        from data.refsegrs_refer import ReferDataset
...
def evaluate(model, data_loader, epoch):
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            total_its += 1
            image, target, sentences, attentions, sentence_raw = data
...
def train_one_epoch(model, contrastive_loss, criterion, optimizer, data_loader, lr_scheduler, epoch, print_freq,
                    iterations):
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    for i, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        optimizer.zero_grad()
        loss = criterion(output, target) + 0.2 * loss_contrast
...
def main(args):
    ...
    print("\n[***] Build Model")
    # print(args.model)
    model = segmentation.__dict__[args.model](pretrained='',
                                              args=args)
    model.cuda()
